...
function renderAbout(){
  ...
  out += '<div class="story-text">FATE: INTO THE PITS\n\nSaltmark is drowning. You run an unlicensed fight den called THE PIT. Recruit fighters, manage rivals and cops and corps, and stage Fight Nights where real stakes ride on real outcomes.\n\nFATE is the only currency, spent and won only at Fight Night and Side Bets. Every match is settled by a real DreamDEX prediction market on Somnia — the outcome is not generated by the game. The odds are real, the payouts vary, and the result comes from the market, not from the Pit.</div>';
